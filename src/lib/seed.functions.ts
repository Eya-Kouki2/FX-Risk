...
const CLIENTS = [
  "International Trading Co.", "Atlas Imports SARL", "Nordea Shipping AB", "Mediterranean Oil SA",
  "Sahara Minerals Ltd", "Volga Agro Group", "Pacific Electronics Inc", "Lusitania Textile",
  "Cairo Cotton Export", "Helvetia Pharma AG", "Dakar Cocoa Union", "Casablanca Steel",
  "Bavarian Motors GmbH", "Andes Mining Corp", "Singapore Spice House",
];
const COUNTERPARTIES = [
  "HSBC London", "BNP Paribas Paris", "JPMorgan New York", "Deutsche Bank Frankfurt",
  "UBS Zurich", "Standard Chartered Dubai", "Citi Singapore", "Société Générale", "Barclays London",
];
const PAIRS: Array<[string, string]> = [
  ["EUR", "USD"], ["USD", "EUR"], ["GBP", "USD"], ["USD", "JPY"],
  ["EUR", "GBP"], ["USD", "CHF"], ["EUR", "JPY"], ["USD", "MAD"], ["EUR", "AED"], ["USD", "CNY"],
];
const RATES: Record<string, number> = {
  "EUR/USD": 1.085, "USD/EUR": 0.922, "GBP/USD": 1.265, "USD/JPY": 149.5,
  "EUR/GBP": 0.858, "USD/CHF": 0.881, "EUR/JPY": 162.3, "USD/MAD": 9.95,
  "EUR/AED": 3.98, "USD/CNY": 7.18,
};
...
export const seedDemoData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    await supabaseAdmin.from("operations").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const amount = isAnomaly && Math.random() < 0.5
      ...
      const status =
        statusRoll < 0.45 ? "pending"
        : statusRoll < 0.7 ? "validated"
        : statusRoll < 0.82 ? "rejected"
        : statusRoll < 0.92 ? "escalated"
        : "settled";
    ...
    const { error } = await supabaseAdmin.from("operations").insert(ops as any);
