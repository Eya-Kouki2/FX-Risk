...
export function OperationDetailDialog({ operationId, onClose }: Props) {
  ...
  const act = async (status: "validated" | "rejected" | "escalated") => {
    if (!op) return;
    const update: TablesUpdate<"operations"> = { status };
    if (status === "validated") {
    ...
    toast.success(
      `Opération ${op.operation_ref} ${status === "validated" ? "validée" : status === "rejected" ? "rejetée" : "escaladée"}`,
    );
    qc.invalidateQueries();
    onClose();
  };
        ...
        {!op ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              ...
              <Info
                label="Taux de marché"
                value={op.market_rate ? Number(op.market_rate).toFixed(4) : "—"}
                mono
              />
              <Info label="Date valeur" value={format(new Date(op.value_date), "PP")} />
              <Info
                label="SWIFT"
                value={op.swift_reference || "Manquant"}
                mono
                danger={!op.swift_reference}
              />
            ...
            {op.rejection_reason && (
              <div className="text-sm bg-destructive/10 rounded-md p-3 border border-destructive/30">
                <div className="text-xs text-destructive uppercase mb-1 font-semibold">
                  Motif de rejet
                </div>
                {op.rejection_reason}
              </div>
              ...
              {opAlerts.length === 0 ? (
                ...
                <ul className="space-y-1.5">
                  {opAlerts.map((a) => (
                    <li
                      key={a.id}
                      className="text-sm flex items-start gap-2 p-2 rounded-md border border-border bg-muted/20"
                    >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...f,
                              swift_reference: e.target.value.toUpperCase(),
                            }))
                          }
                        />
                      </div>
            ...
            {canValidate && (
              <div className="border-t border-border pt-4 space-y-2">
                ...
                <Input
                  ...
                  placeholder="ex: Référence SWIFT invalide"
                />
              </div>
            )}
            ...
            <DialogFooter className="gap-2 flex-wrap">
              {canCorrect && editing && (
                <Button onClick={resubmit}>
                  <Save className="h-4 w-4 mr-1.5" /> Resoumettre
                </Button>
              )}
              {canValidate && (
                <>
...
function Info({
  ...
  return (
    <div>
      ...
      <div
        className={`mt-0.5 ${mono ? "font-mono text-sm" : "text-sm font-medium"} ${danger ? "text-destructive" : ""}`}
      >
        {value}
      </div>
    </div>
