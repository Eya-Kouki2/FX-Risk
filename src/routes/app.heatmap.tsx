...
type Cell = { prob: "Low" | "Medium" | "High"; impact: "Low" | "Medium" | "High"; severity: "low" | "moderate" | "high" | "critical"; count: number };
...
function HeatmapPage() {
  const { data: ops = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("operations").select("risk_score, amount, risk_level, client_name, operation_ref, buy_currency, sell_currency");
      return data ?? [];
    },
  });
  ...
  const matrix: Cell[][] = PROB.map((p) =>
    IMPACT.map((i) => ({
      ...
      count: ops.filter((o) => classifyProb(o.risk_score) === p && classifyImpact(Number(o.amount)) === i).length,
    }))
  );

  const top = [...ops].sort((a, b) => b.risk_score - a.risk_score).slice(0, 6);
          ...
          <div className="flex">
            ...
            <div className="flex-1">
              <div className="grid grid-cols-3 gap-2">
                {[...matrix].reverse().map((row, ri) => row.map((cell, ci) => {
                  ...
                  return (
                    <div key={`${ri}-${ci}`}
                      ...
                      <div className="text-[10px] text-muted-foreground mt-1">P:{cell.prob} · I:{cell.impact}</div>
                    </div>
                  );
                }))}
              </div>
              <div className="grid grid-cols-3 mt-3 text-xs text-muted-foreground text-center">
                {IMPACT.map((i) => <div key={i}>{i}</div>)}
              </div>
            ...
            {top.map((o, i) => (
              <div key={i} className="flex items-center justify-between text-sm border-b border-border last:border-0 pb-2">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground font-mono">{o.operation_ref} · {o.buy_currency}/{o.sell_currency}</div>
                </div>
                <div className={`text-sm font-mono font-bold ${
                  o.risk_level === "critical" ? "text-risk-critical" :
                  o.risk_level === "high" ? "text-risk-high" :
                  o.risk_level === "moderate" ? "text-risk-moderate" : "text-risk-low"
                }`}>{o.risk_score}</div>
              </div>
            ))}
