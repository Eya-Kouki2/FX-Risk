  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Platform members and their assigned roles</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="stat-card"><div className="text-xs text-muted-foreground uppercase tracking-wide">Total users</div><div className="text-2xl font-display font-bold mt-2">{profiles.length}</div></div>
            ...
            <tbody>
              {profiles.map((p) => (
                <tr key={p.id} className="border-t border-border hover:bg-muted/30">
                  <td className="px-4 py-2.5 font-medium">{p.full_name ?? "Unnamed"}</td>
