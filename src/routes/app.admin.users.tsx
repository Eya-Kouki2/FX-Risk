...
function UsersAdmin() {
  const { data: profiles = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (profs ?? []).map((p) => ({
        ...p,
        role: roleMap.get(p.id) ?? ("front_office" as AppRole),
      }));
    },
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Membres de la plateforme et leurs rôles assignés
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="stat-card">
          ...
          <div className="text-2xl font-display font-bold mt-2">{profiles.length}</div>
        </div>
        {(["admin", "manager", "risk_team", "back_office", "front_office"] as AppRole[]).map(
          (r) => (
