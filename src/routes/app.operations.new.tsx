...
function NewOperation() {
  ...
  const submit = async (status: "draft" | "pending") => {
    ...
    try {
      ...
      if (isNewClient) {
        ...
        const { data: newClient, error: clientErr } = await supabase
          ...
          .insert({
            ...
            matricule_fiscal: form.new_client_type === "societe" ? (form.new_client_mf || null) : null,
            created_by: user.id
          })
          .select()
          .single();
      ...
      if (!form.exchange_rate || Number(form.exchange_rate) <= 0) throw new Error("Taux de change invalide.");
      if (!form.counterparty || form.counterparty.length < 2) throw new Error("Contrepartie requise.");
        ...
        <section className="stat-card space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <label htmlFor="new-client" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          ...
          {isNewClient ? (
            <div className="p-4 bg-muted/30 rounded-lg border space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label={form.new_client_type === "particulier" ? "Nom et Prénom" : "Raison Sociale"} required>
                  <Input
                    value={form.new_client_name}
                    onChange={(e) => update("new_client_name")(e.target.value)}
                    placeholder={form.new_client_type === "particulier" ? "Ex: Ali Ben Salah" : "Ex: Société Tunisienne de Textile"}
                        ...
                        <CommandGroup>
                          {clients?.map((c) => (
                            ...
                            >
                              ...
                              />
                              {c.name} {c.cin ? <span className="ml-2 text-muted-foreground">(CIN: {c.cin})</span> : ""}
                            </CommandItem>
                          ))}
                        </CommandGroup>
                      </CommandList>
