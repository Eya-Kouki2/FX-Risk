...
function ReportsPage() {
  const { data: ops = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("operations").select("*").order("created_at", { ascending: false }))
        .data ?? [],
  });
  const { data: alerts = [] } = useQuery({
    queryKey: ["reports-alerts"],
    queryFn: async () =>
      (await supabase.from("alerts").select("*").order("created_at", { ascending: false })).data ??
      [],
  });
  ...
  const exportCsv = () => {
    const headers: Array<keyof Tables<"operations">> = [
      "operation_ref",
      "client_name",
      "buy_currency",
      "sell_currency",
    ...
    const rows = ops.map((o) => headers.map((h) => JSON.stringify(o[h] ?? "")).join(","));
    const csv = [headers.join(","), ...rows].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `fx-operations-${format(new Date(), "yyyy-MM-dd")}.csv`;
    a.click();
  ...
  const reports = [
    ...
    },
    {
      ...
      icon: FileText,
      actions: [{ label: "CSV", icon: Download, fn: exportCsv }],
    },
  ];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Exporter les données opérationnelles et les synthèses de risques
        </p>
      </div>
